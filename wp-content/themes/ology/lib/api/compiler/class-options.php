 ...
 */
final class torbara_tt_Compiler_Options {
	...
	public function __construct() {
		...
		add_action( 'torbara_field_flush_cache', array( $this, 'option' ) );
		add_action( 'torbara_field_descriptiontorbara_tt_compile_all_styles_append_markup', array( $this, 'maybe_disable_style_notice' ) );
		add_action( 'torbara_field_descriptiontorbara_tt_compile_all_scripts_group_append_markup', array( $this, 'maybe_disable_scripts_notice' ) );
	...
	public function register() {
		...
		if ( torbara_get_component_support( 'wp_styles_compiler' ) )
			$fields = array_merge( $fields, array(
				array(
					'id' => 'torbara_compile_all_styles',
					'label' => false,
		...
		if ( torbara_get_component_support( 'wp_scripts_compiler' ) )
			$fields = array_merge( $fields, array(
				array(
					'id' => 'torbara_compile_all_scripts_group',
					'label' => esc_html__( 'Compile all WordPress scripts', 'torbara' ),
					'type' => 'group',
					'fields' => array(
						array(
							'id' => 'torbara_compile_all_scripts',
							'type' => 'activation',
							'default' => false
						),
						array(
							'id' => 'torbara_compile_all_scripts_mode',
							'type' => 'select',
		...
		torbara_register_options( $fields, 'torbara_settings', 'compiler_options', array(
	...
	public function flush() {
		...
		if ( !torbara_post( 'torbara_flush_compiler_cache' ) )
			return;

		torbara_remove_dir( torbara_get_compiler_dir() );
	...
	public function option( $field ) {
		...
		if ( $field['id'] !== 'torbara_compiler_items' )
			return;

		echo '<input type="submit" name="torbara_flush_compiler_cache" value="' . esc_html__( 'Flush assets cache', 'torbara' ) . '" class="button-secondary" />';
	...
	public function maybe_disable_style_notice() {
		...
		if ( get_option( 'torbara_compile_all_styles' ) && torbara_tt_is_compiler_dev_mode() )
	...
	public function maybe_disable_scripts_notice() {
		...
		if ( get_option( 'torbara_compile_all_scripts' ) && torbara_tt_is_compiler_dev_mode() )
...
new torbara_tt_Compiler_Options();
