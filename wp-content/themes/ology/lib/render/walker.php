...
class ology_tt_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( $depth < $args->ology_start_level )
			return;
		...
		if ( $type === 'navbar' && $depth === $args->ology_start_level ) {
			...
			// Open sub_menu wrap.
			$output .= ology_open_markup( "ology_sub_menu_wrap[_{$type}]{$location_subfilter}", 'div', 'class=uk-dropdown uk-dropdown-navbar', $depth, $args );
		...
		// Open sub_menu.
		$output .= ology_open_markup( "ology_sub_menu[_{$type}]{$location_subfilter}", 'ul', $attr, $depth, $args );
	...
	function end_lvl( &$output, $depth = 0, $args = array() ) {
		...
		// Close sub_menu.
		$output .= ology_close_markup( "ology_sub_menu[_{$type}]{$location_subfilter}", 'ul' );

		// Close sub_menu wrap.
		if ( $type === 'navbar' && $depth === $args->ology_start_level )
			$output .= ology_close_markup( "ology_sub_menu_wrap[_{$type}]{$location_subfilter}", 'div', $depth, $args );
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $depth == $args->ology_start_level && in_array( 'menu-item-has-children', $classes ) ) {

			$item_attr['class'][] = 'uk-parent';

			if ( ology_get( 'ology_type', $args ) == 'navbar' ) {
		...
		$output .= ology_open_markup( "ology_menu_item[_{$item_id}]", 'li', $item_attr, $item, $depth, $args );

			$item_output = $args->before;

				$item_output .= ology_open_markup( "ology_menu_item_link[_{$item_id}]", 'a', $link_attr, $item, $depth, $args );

					$item_output .= ology_output( "ology_menu_item_text[_{$item_id}]", $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after );

					if ( isset( $child_indicator ) ) {

						$item_output .= ology_open_markup( "ology_menu_item_child_indicator[_{$item_id}]", 'i', array( 'class' => 'uk-icon-caret-down uk-margin-small-left' ), $item, $depth, $args );
						$item_output .= ology_close_markup( "ology_menu_item_child_indicator[_{$item_id}]", 'i', $item, $depth, $args );

					}

				$item_output .= ology_close_markup( "ology_menu_item_link[_{$item_id}]", 'a', $link_attr, $item, $depth, $args );
	...
	function end_el( &$output, $item, $depth = 0, $args = array() ) {
		...
		$output .= ology_close_markup( "ology_menu_item[_{$item_id}]", 'li', $item, $depth, $args );
