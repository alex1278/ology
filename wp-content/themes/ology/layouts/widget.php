...
// Set default panel
if ($panel == '' && in_array($widget->position, array('footer', 'top-a', 'top-b', 'top-c', 'top-d', 'top-e', 'top-f', 'top-g', 'bottom-a', 'bottom-b', 'bottom-c', 'bottom-d', 'bottom-e', 'bottom-f', 'bottom-g', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
    $panel = $this['config']->get("panel_default.{$widget->position}.panel", '');
}
// Set panel for specific positions
else if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'social-sidebar', 'offcanvas'))) {
	$panel = 'uk-panel';
}
...
// Set title
if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l'))) {
	$title = '';
} elseif ($title && !($widget->position == 'menu')) {
	$title = '<h3 class="'.($title_size ? $title_size : 'uk-panel-title').'"><span>'.$icon.$title.'</span></h3>';
}
...
if ($widget->menu) {
	...
	if (isset($params['menu'])) {
		...
		//$widget->nav_settings["modifier"] = "uk-nav-offcanvas";
                $widget->nav_settings["accordion"] = true; //ang
	} else {
		$renderer = 'nav';
...
// Render widget
if (in_array($widget->position, array('breadcrumbs', 'extra-toolbar', 'logo', 'logo-small', 'search', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
	echo $content;
    ...
    if (isset($params['animation']) && $params['animation'] !="") { $tm_animation = 'data-uk-scrollspy="{cls:\''.$params['animation'].'\''.(isset($params['animation_repeat']) ?', repeat: true':'').((isset($params['delay'])) ?', '.$params['delay'] :'').'}"'; }
