...
class ANGProductsSliderPack extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'ANG-WooCommerce-Slider-Pack', // Base ID
            __('ANG WooCommerce Slider Pack', 'ang-plugins'), // Name
            array( 'description' => __( 'WooCommerce Products Slider pack widget with customizable settings and various templates.', 'ang-plugins' ), ) // Args
        );
    }
    ...
    function form($instance) { ?>
        ...
        <?php $title = isset( $instance['title']) ? esc_attr( $instance['title'] ) : "Latest products"; ?>
        <p>
...
<!--             Returns  WooCommerce 'product' taxonomy -->
        
        <?php $ProductCat = isset( $instance['ProductCat'] ) ? $instance['ProductCat'] : ''; ?>
        ...
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('ProductCat'); ?>">
                <?php esc_html_e('Product category :', 'ang-plugins'); ?>
                    <select class="widefat" 
                          id="<?php echo esc_attr($this->get_field_id('ProductCat')); ?>" 
                          name="<?php echo esc_attr($this->get_field_name('ProductCat')); ?>" >
                      <option value="" <?php if('' == $instance['ProductCat']){echo 'selected=""';} ?>><?php esc_html_e('-- All Products --', 'ang-plugins'); ?></option> <?php
                foreach ($tax_events as $tax_event){
                    ?><option value="<?php echo esc_attr($tax_event->term_id); ?>" <?php if($tax_event->term_id==$instance['ProductCat']){echo 'selected=""';} ?>><?php echo $tax_event->name; ?></option><?php
                }
                ?>
                </select>
            </label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('page_link_id'); ?>">
                ...
                foreach ($pages as $page){ ?>
                    <option value="<?php echo esc_attr($page->ID); ?>" <?php if($page->ID==$instance['page_link_id']){echo 'selected=""';} ?>><?php echo $page->post_title; ?></option>
                <?php } ?>
                </select>
            </label>
        </p>
...
<!--        Select view type, template  -->
        ...
        <?php
            ...
            $temp_dir_path = $plugin_dir.'woo-product-slides-templates/'; // path to templates folder
            $templates = array();
            ...
            <label for="<?php echo $this->get_field_id('view_template'); ?>">
                ...
                <?php
                    foreach ($templates as $key => $template){ ?>
                        <option value="<?php echo esc_attr($template); ?>" <?php if($template == $instance['view_template']){echo 'selected=""';} ?>>
                            <?php 
...
<!--   Checkox Show the post excerpt -->
        ...
        <p>
            <input type="checkbox" id="<?php echo $this->get_field_id('p_excpt'); ?>" name="<?php echo $this->get_field_name('p_excpt'); ?>" <?php if ($instance['p_excpt']) echo 'checked'; ?> />
...
<!--   Checkox Show only featured or sticky -->
    ...
    function widget($args, $instance) {
         ...
         */
        if( $instance['ProductCat'] !=''){
            $args['tax_query'][] = array(
                        'taxonomy'  => 'product_cat',
                        'field'     => 'term_id',
                        'terms'     => $instance['ProductCat'],
            );
        }
    ...
    // define plugin template path
        ...
        // code here template
        include $plugin_dir.'woo-product-slides-templates/'.$instance['view_template']; // path to templates
...
******* apply style dir
*****/
function load_ang_woo_product_slides_css() {
    $plugin_url = plugin_dir_url( __FILE__ );

    wp_enqueue_style('css-ang-testimonials', $plugin_url . 'woo-product-slides-css/woo-product-slides.css' );
}
add_action('wp_enqueue_scripts', 'load_ang_woo_product_slides_css' );

// activate widget if Woocommerce is active
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
    add_action('widgets_init', create_function('', 'return register_widget("ANGProductsSliderPack");'));
}
?>