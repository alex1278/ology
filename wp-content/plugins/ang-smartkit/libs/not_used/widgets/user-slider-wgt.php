...
class ANGAgentsSlider extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            ...
            esc_html__('ANG Agents Slider', 'ang-plugins'), // Name
            array( 'description' => esc_html__( 'Widget, displays property agents in slider style.', 'ang-plugins' ), ) // Args
        );
    }
    ...
    function form($instance) { ?>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('wp_roles'); ?>">
                <?php esc_html_e('Select user role to display:', 'ang-plugins'); ?>
                    <?php
                ...
                foreach ($roles as $key=>$role){
                    ?><option value="<?php echo esc_attr($key); ?>" <?php if($key==$instance['wp_roles']){echo 'selected=""';} ?>><?php echo $role; ?></option><?php
                }
                ?>
                </select>
            </label>
...
<!--        Pause on hover-->
    ...
    function widget($args, $instance) {
       ...
       ?>
<div class="ang-user-slider role-<?php echo $instance['wp_roles']; ?> order_by-<?php echo $instance['p_order_by'] ; ?> underlined-header">
    <div class="uk-slidenav-position" data-uk-slider="{<?php echo esc_attr(implode(', ', $slideshow_cfg)); ?>}">
     ...
     <?php
            foreach( $users as $user ){
                ...
                ?>
                <?php if ( $facebook != '' ) { 
                    $facebook_check ="<a rel='nofollow' class='uk-icon-small uk-icon-hover uk-icon-facebook' href='$facebook'></a>";
                    }
                    
                if ( $twitter != '' ) {
                    $twitter_check = "<a rel='nofollow' class='uk-icon-small uk-icon-hover uk-icon-twitter' href='$twitter'></a>";
                    }
                    
                if ( $google != '' ) { 
                    $google_check = "<a rel='nofollow' class='uk-icon-small uk-icon-hover uk-icon-google-plus' href='$google'></a>";
                    }
                    
                if ( $linkedin != '' ) {
                    $linkedin_check = "<a rel='nofollow' class='uk-icon-small uk-icon-hover uk-icon-linkedin' href='$linkedin'></a>";
                    }
                    
                if ( $mobile != '' ) { 
                    $mobile_check = "<p class=' ang-agent-mobile'>$mobile</p>";
                    }
                    
                if ( $email != '' ) {
                    $email_check = "<p class='ang-agent-email'>$email</p>";
                    }
                    
                if ( $position != '' ) {
                    $position_check = "<p class='tm-agent-position'>$position</p>";
                    }
                            ...
                            <div class="uk-height-1-1 tm-overlay-agent">
                                <div class="uk-height-1-1 uk-position-relative uk-vertical-align">
                                    <div class="tm-border-block uk-vertical-align-middle  uk-container-center">
                                        <?php echo $mobile_check; ?>
                                        <?php echo $email_check ;?>
                                        <div class="uk-width-1-1 uk-text-center ang-agent-social ">
                                            <?php echo $facebook_check.$twitter_check.$google_check.$linkedin_check ;?>
                                         </div>
...
add_action('widgets_init', create_function('', 'return register_widget("ANGAgentsSlider");')); ?>